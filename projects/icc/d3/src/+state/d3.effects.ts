    ...
    this.actions$.pipe(
      ...
      concatMap(({ d3Config }) => {
        return this.d3Service.getRemoteD3Config(d3Config).pipe(
          map((d3Config) => {
            if (d3Config.remoteFieldsConfig) {
        ...
        return this.d3Service.getD3ChartConfigs(d3Config).pipe(
          map((d3Fields) => {
            if (d3Config.remoteD3Data) {
              this.store.dispatch(d3Actions.loadD3ChartConfigsSuccess({ d3Config, d3Fields }));
              return d3Actions.getD3Data({ d3Config });
            } else {
              return d3Actions.loadD3ChartConfigsSuccess({ d3Config, d3Fields });
            }
          }),
        );
      }),
    ...
    this.actions$.pipe(
      ...
      switchMap(({ d3Config }) => {
        return this.d3Service.getD3Data(d3Config).pipe(
          map(({ d3Config, d3Data }) => {
            return d3Actions.getD3DataSuccess({ d3Config, d3Data });
          }),
        );
      }),
    ),
